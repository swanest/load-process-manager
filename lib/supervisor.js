...
module.exports = function (opts) {
    ...
    function handleMessage(worker, msg) {
        ...
        switch (msg.event) {
            case C.READY:
                debug('Worker ready to work' + worker.process.pid);
                worker.ready = true;
                worker.process.send({
                    event: C.DECLARE_PID,
                    pid: process.pid
            ...
            case C.HEALTH:
                ...
                worker.handledSockets = msg.handledSockets;
                break;
            case C.BUSY:
                worker.ready = false;
                break;
    ...
    function spreadRequests() {
        ...
        if (r.socket != void 0) {
            availableWorkers = _.sortBy(workers, 'handledSockets');
            w = _.first(availableWorkers);
            w.handledSockets++;
        }
        if (r.timeout != -1)
        ...
        if (r.stream != void 0) {
            ...
            var streamToChild = net.connect('/tmp/lpm-child-' + w.process.pid + '.sock');
    ...
    sp.enqueue = function enqueue(req, streamOrSocket, options, cb) {
        if (arguments.length < 4 && !(streamOrSocket instanceof net.Socket) && streamOrSocket != void 0 && !_.isFunction(streamOrSocket.pipe)) {
            cb = options;
            options = streamOrSocket;
        }
        if (cb == void 0) {
            cb = _.noop();
        }
        ...
        if (streamOrSocket && _.isFunction(streamOrSocket.pipe) && !(streamOrSocket instanceof net.Socket)) {
            construct.stream = streamOrSocket;
        } else if (streamOrSocket instanceof net.Socket) {
            construct.socket = streamOrSocket;
        }
